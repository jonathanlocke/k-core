...
object Bits
{
    ...
    fun oneBits(count: Countable<*>): Long
    {
        var bit = 1L
        var value = 0L
        for (i in count.count().asSequence())
        {
            value = value or bit
            bit = bit shl 1
        }
     ...
     */
    fun parseBits(text: String, handler: ProblemHandler<Long> = throwProblem()): Long
    {
        var value = 0L
        for (i in text.count().asInts())
        {
            value = when (text[i])
            {
                '1' -> value or 1
                ...
                else -> handler.reportProblem("$text is not a binary number") ?: -1
            }
            value = value shl 1
        }
        return value
